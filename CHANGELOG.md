...
- Channels: `channels.<channel>.accounts` keys are now validated, and a config carrying an invalid one FAILS TO LOAD — the gateway refuses to start rather than starting with the account block silently ignored. Two rules, applied according to how each channel's resolver looks an account up:
  - `discord`, `slack`, `imessage`, `whatsapp`, `googlechat`, `bluebubbles` (and `signal`, as of the previous entry) normalize the requested account id and then do an EXACT lookup, so their keys must already be written in normalized form — lowercase, only `a-z`, `0-9`, `_` and `-`, at most 64 characters. Keys like `Alerts`, `ops.eu` or `DEFAULT` were never matched at runtime: their settings, credentials included, were already being silently ignored while the account fell back to the CHANNEL-level bot token / API key / service account. Rename them to their normalized spelling (`alerts`, `ops-eu`, `default`); the error message names the replacement.
  - `telegram` and `irc` match non-normalized keys at runtime, so their keys are NOT required to be normalized and nothing that worked before stops working. They reject only `__proto__`, which cannot exist as a plain object key: the entry is dropped while the config is parsed, so no lookup can ever find it, the account silently presents the channel-level credential, and the next config write erases the block.
    Remediation for either rule: rename the offending key. The validation error names the exact config path (`channels.<channel>.accounts.<key>`) and, for the normalization rule, the replacement key.
...
- Channels/Config: account lookups now read the `accounts` record own-property-only. `accounts["constructor"]` returns the global `Object` on any config that does not declare that account, and `accounts["__proto__"]` returns `Object.prototype`; both are truthy, so every resolver that tried a direct hit before falling back to a normalized-key scan skipped the scan and never found the account the operator did configure. Telegram, IRC, Matrix and Nextcloud Talk therefore presented the CHANNEL-level bot token / server password / access token / bot secret for an account whose own credential was right there in the config. Fixed in the account and token resolvers for those channels plus `resolveChannelCapabilities`; the same read-guard is applied to Discord, Slack, iMessage and WhatsApp as defense-in-depth.
- Channels/Config: `configWrites` no longer fails OPEN for an account named `constructor`. The lookup used `accountId in accounts`, which is true on every object, so a per-account `configWrites: false` was resolved against the global `Object`, found no `configWrites`, and fell through to the channel-level default.
- Google Chat/Config: `channels.googlechat.serviceAccount` is now marked sensitive at both the channel and per-account level, in both accepted shapes. The service-account JSON — `private_key` included — was previously emitted verbatim in redacted config snapshots, because neither `serviceAccount` nor `private_key` matches the name-pattern fallback used when a field is not explicitly registered.
- Config/Redaction: raw-source redaction now also matches the escaped spelling of a secret. Sensitive values are collected from the PARSED config, so a secret containing an escaped character — a newline above all, which every GCP private key has — did not match the JSON5 source text and survived in `snapshot.raw` even when the parsed copy was redacted.
